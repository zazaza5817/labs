...
def parabolic_integral(f, a, b, n):
    """
   Вычисляет интеграл функции f от a до b с использованием метода парабол.

   :param f: функция, интеграл которой вычисляется
   :param a: начало интервала интегрирования
   :param b: конец интервала интегрирования
   :param n: количество сегментов, на которые разбивается интервал интегрирования
   :return: приближенное значение интеграла
   """
    # Вычисляем шаг, на который разбивается интервал интегрирования
    h = (b - a) / n

    # Инициализируем сумму
    integral = 0

    # Вычисляем значения функции в узлах и на параболической кривой
    for i in range(n):
        x = a + i * h
        y = f(x)
        x1 = a + (i + 0.5) * h
...
def main():
    ...
    value1_3 = parabolic_integral(f, a, b, n1)

    value2_1 = right_rectangles(f, a, b, n2)
    value2_2 = trapezoids(f, a, b, n2)
    value2_3 = parabolic_integral(f, a, b, n2)
    ...
    abs_error3_1, rel_error3_1 = error_calculation(f_integral, a, b, value1_3)
    abs_error3_2, rel_error3_2 = error_calculation(f_integral, a, b, value2_3)
    ...
    print(f"|{'':-^20s}|{'':-^20s}|{'':-^20s}|")
    print(f"|{'Парабол':^20s}|{value1_3:^20.7g}|{value2_3:^20.7g}|")
    print("-" * 64)
    # Конец таблицы
    ...
    # print(f"Абсолютная погрешность метода трапеций: {abs_error1_2:.6g}, {abs_error2_2:.6g}")
    # print(f"Относительная погрешность метода трапеций (в %): {rel_error1_2:.6g}, {rel_error2_2:.6g}")
    min_error = min(rel_error1_1, rel_error2_1, rel_error1_2, rel_error2_2)
    print(f"Минимальная относительная погрешность метода правых прямоугольников и трапеций: {min_error:.6g}")
    print(f"Абсолютная погрешность метода парабол: {abs_error3_1:.6g}, {abs_error3_2:.6g}")
    print(f"Относительная погрешность метода парабол (в %): {rel_error3_1:.6g}, {rel_error3_2:.6g}")
